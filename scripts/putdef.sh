#!/bin/sh
echo "#########################	ERNIE-14 Get report definition detail	######################### "
echo "As a Report Administrator, I want to be able to replace a report definition on the server, "
echo "without negatively impacting any jobs in process that use that report definition."
echo ""
echo ""
curl -v -X PUT -T $2 -H "Content-type: application/rptdesign+xml"\
 -H 'DefinitionEntity:{"createdDate":null,"defId":"'$3'","createdUser":"default","paramNames":null,"params":null,"defDescription":"test description","unsupportedReportTypes":null}' --header "Accept: application/vnd.ksmpartners.ernie+json" --header "Authorization: SAML tVdbc7JMEr73V1i+lynDQVGxolXDyYCCQRGjN1scRkA5yQxC/PWLmFgxm7yb3arviuqe7qef7p5D\
