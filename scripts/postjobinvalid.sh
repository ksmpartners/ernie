#!/bin/sh
echo "######################### ERNIE-57 Run reports for invalid request   ######################### "
echo "As a Client Application Developer, I want to be able to run a report asynchronously, passing"
echo "in the values of required parameters, if any. If a request is sent by the client that is"
echo "malformed or it references a nonexistent definition file a notification should be returned"
echo "(HTTP 400)."
echo ""
echo ""
echo ""
rptType=$(echo $1 | tr '[:lower:]' '[:upper:]') 
curl -v -X POST -d '{"defId":"","rptType":"","retentionDays":'$2',"reportParameters":null}' -H "Content-type: application/json" --header "Accept: application/vnd.ksmpartners.ernie+json" --header "Authorization: SAML tVdbc7JMEr73V1i+lynDQVGxolXDyYCCQRGjN1scRkA5yQxC/PWLmFgxm7yb3arviuqe7qef7p5D\
