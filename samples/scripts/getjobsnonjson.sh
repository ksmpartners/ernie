#!/bin/sh
echo "#########################		ERNIE-89 List current jobs service returns a 406 for non-JSON requests	######################### "
echo "As a Client Application Developer, I want to be able to retrieve a list of all of the jobs "
echo "known to the server."
echo ""
echo "If the client does not accept the application JSON type in its accepts header, throw a 406 HTTP response code."
echo ""
echo ""
wget --header "Accept: application/vnd.ksmpartners.ernie+xml" --header "Authorization: SAML tVdbc7JMEr73V1i+lynDQVGxolXDyYCCQRGjN1scRkA5yQxC/PWLmFgxm7yb3arviuqe7qef7p5D\84SsKKSHACGY4SCJm2UUxmhYa0etPIuHiYUCNIytCKIhdoZLoM6G9CM5tD5cWu8+JRq1fIzTIUEU\
